...
void	ft_cursor_offset(int x, int y, t_data *mlx)
{
	...
	mlx->min_xr += xr_zoom_diff * mlx->x_100;
	mlx->max_xr -= xr_zoom_diff * (1 - mlx->x_100);
	mlx->y_100 = (double)y / (double)HEIGHT;
	yi_zoom_diff = ((fabs(mlx->min_yi)+fabs(mlx->max_yi)) - (WIDTH/(mlx->scale*mlx->zoom)));
	mlx->min_yi -= yi_zoom_diff * (1 - mlx->y_100);
	mlx->max_yi += yi_zoom_diff * mlx->y_100;
	printf("%f\n", xr_zoom_diff); // PRINT VALUES AND FIND OUT WHAT'S WRONG
	printf("%f\n", yi_zoom_diff); // PRINT VALUES AND FIND OUT WHAT'S WRONG
}
